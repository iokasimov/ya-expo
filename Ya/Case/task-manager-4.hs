...
start = to @(Scrolling List) `ha` Nonempty @List @Task
 `ha` Item (TODO `lu` "Apply to that new position") `ha` Maybe `ha` Next
 `ha` Item (TODO `lu` "Find a way to fix ligatures")`ha` Maybe `ha` Next
 `ha` Item (TODO `lu` "Organize a boardgame session") `ha` Maybe `ha` Next
 `ha` Item (DONE `lu` "Buy a water gun for Songkran") `ha` Maybe `hv` Last
